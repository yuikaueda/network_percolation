...
axe.plot(l2, p2, 's', markersize=4, c='orange', label = r'$k=2.0, \theta =0.1$')#'$k_{on}^{c}=5,k_{off}=0.2$')
axe.plot(l3, p3, 'v', markersize=4, c='dodgerblue', label = r'$k=3.0, \theta =0.1$')#'$k_{on}^{c}=10,k_{off}=0.2$')
axe.plot(l4, p4, 'p',  markersize=4, c='seagreen', label =r'$k=4.0, \theta =0.1$')#'$k_{on}^{c}=20,k_{off}=0.2$')
...
fig.savefig("perb_gamma_k2.0_sig0.1_center5_t1000_nbest200.png")
plt.show()
