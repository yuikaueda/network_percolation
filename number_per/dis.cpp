...
int main(void){
  ...
  FILE* fp0;
  char filename[256];
  sprintf(filename,"ndis_n%d_t%d_nbest%d.dat",N,t_max,N_best);
...
*/
    ...
    for(int dy_num=0; dy_num<divide; dy_num++){
        for(int dx_num=0; dx_num<divide; dx_num++){
          fprintf(fp0,"%2d\t%2d\t%d\n",dx_num,dy_num,p_pos[dx_num][dy_num].cnum);
          printf("%d\t%d\t%d\n",dx_num,dy_num,p_pos[dx_num][dy_num].cnum);
        }
    }
...
*/
  ...
  for(int i = 0; i < t_max; i++){
    ...
    double ave_sd = sd/(double)step;
	  ...
	  //fprintf(fp0, "%d\t%f\t%f\t%f\t%f\t%f\n",i,p_a,n_c,n_cc,ave_dd,ave_sd);
	  //printf("t=%d\tP_a=%f\tnc=%f\tncc=%f\tdens=%f\tSD=%f\n",i,p_a,n_c,n_cc,ave_dd,ave_sd);
  }
